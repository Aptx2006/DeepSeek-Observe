...
如果还在阿里云百炼平台下使用，直接改 `--model` 的值就能切换模型名；前提是该模型在你的账号下可用。

默认选 `qwen3.6-27b` 的原因：

- 阿里云百炼新用户通常有免费 token，适合先低成本试跑
- 这是 Dense 模型，视觉理解能力和综合表现比较稳
- 成本、效果和可用性之间比较均衡，适合作为默认值

脚本会按图片内容、模式和模型做本地缓存，重复分析同一张图时会直接复用上次的 Markdown。
调用结束后，会在终端输出本次使用的模型和 token 消耗。
本地缓存默认最多保留 500 条，超出后会自动删除旧记录；`CACHE_MAX_ENTRIES=0` 时表示不限制数量。
